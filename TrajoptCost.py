...
class ArmCost(TrajoptCost):
	...
	def symbolic_cost(self, control=True):
		...
		delta_x = Matrix([pos_ee, vel_ee]) - xg # Matrix(self.xg)
	...
	def symbolic_cost_eval(self, control=True):
		...
		delta_x = state - xg #Matrix(self.xg)
		cost=0.5*delta_x.T@Q@delta_x
		...
		if control:
			...
			to_return= lambdify([q1,q2,q1_dot ,q2_dot, u1, u2,xg],delta_x, "numpy")
	...
	# 	currQ = self.get_currQ(u,timestep)
	# 	new_x=np.array([np.cos(x[0]) + np.cos(x[0]+x[1]), np.sin(x[0]) + np.sin(x[0]+x[1]), x[2]*(-np.sin(x[0]) - np.sin(x[0]+x[1])) - x[3]*np.sin(x[0]+x[1]), x[2]*(np.cos(q1) + np.cos(x[0]+x[1])) + x[3]*np.cos(x[0]+x[1])])
	# 	cost=cost=(new_x-self.xg)@currQ@(new_x-self.xg).transpose()
	# 	if u is None:
	# 		return cost
	# 	else:
	# 		cost+=u@self.R@u.transpose()
	# 		return cost
	...
	def symbolic_gradient_eval(self,control=True):
		...
		if(control):
			cost=self.cost_control_in
			gradient = Matrix(BlockMatrix([diff(cost, q1), diff(cost, q2), diff(cost, q1_dot), diff(cost, q2_dot), diff(cost, u1), diff(cost, u2)]))
			to_return= lambdify([q1,q2,q1_dot ,q2_dot,u1, u2, Q,R, xg],gradient, "numpy")
			return to_return
		else:
			cost=self.cost_control_off
			gradient = Matrix(BlockMatrix([diff(cost, q1), diff(cost, q2), diff(cost, q1_dot), diff(cost, q2_dot)]))
			return  lambdify([q1,q2,q1_dot ,q2_dot, Q, xg],gradient, "numpy")
	...
	# 		gradient_val= symbolic_grad(x[0], x[1],x[2], x[3],currQ, self.xg)
	# 	else:
	...
	# 		gradient_val= symbolic_grad(x[0], x[1],x[2], x[3], u[0], u[1],currQ, self.R, self.xg)
