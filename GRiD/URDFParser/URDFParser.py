...
class URDFParser:
    ...
    def parse(self, filename, alpha_tie_breaker = False):
        try:
            # parse the file
            urdf_file = codecs.open(filename, 'r', errors = 'ignore')
            # urdf_file = open(filename, "rb")
            ...
            # self.soup = BeautifulSoup(urdf_file.read(),"xml").find("robot")
            #set up the robot object
            self.robot = Robot(self.soup.get("name"))
            # collect links
            self.parse_links()
            #collect joints
            self.parse_joints()
    ...
    def parse_links(self):
        lid = 0
        for raw_link in self.soup.findall('.//link'):
            # construct link object
            name = raw_link.get("name")
            curr_link = Link(name, lid)
            lid += 1
            ...
            if raw_origin is None:
                ...
                curr_link.set_origin_xyz([float(i) for i in raw_origin.get("xyz", "0 0 0").split()])
                curr_link.set_origin_rpy([float(i) for i in raw_origin.get("rpy", "0 0 0").split()])
            ...
            if raw_inertial is None:
                ...
                # get mass and inertia values
                raw_inertia = raw_inertial.find("inertia")
                curr_link.set_inertia(
                    float(raw_inertial.find("mass").get("value", "0")),
                    float(raw_inertia.get("ixx", "0")),
                    float(raw_inertia.get("ixy", "0")),
                    float(raw_inertia.get("ixz", "0")),
                    float(raw_inertia.get("iyy", "0")),
                    float(raw_inertia.get("iyz", "0")),
                    float(raw_inertia.get("izz", "0"))
                )
    ...
    def parse_joints(self):
        jid = 0
        for raw_joint in self.soup.findall('.//joint'):
            # construct joint object
            curr_joint = Joint(raw_joint.get("name"), jid, \
                                raw_joint.find("parent").get("link"), \
                                raw_joint.find("child").get("link"))
            
            jid += 1
            # get origin position and rotation
            curr_joint.set_origin_xyz(self.to_float(raw_joint.find("origin").get("xyz").split(" ")))
            curr_joint.set_origin_rpy(self.to_float(raw_joint.find("origin").get("rpy").split(" ")))
            # set joint type and axis of motion for joints if applicable
            raw_axis = raw_joint.find("axis")
            if raw_axis is None:
                curr_joint.set_type(raw_joint.get("type"))
            else:
                curr_joint.set_type(raw_joint.get("type"),self.to_float(raw_axis.get("xyz").split(" ")))
            raw_dynamics = raw_joint.find("dynamics")
            if raw_dynamics is None:
                curr_joint.set_damping(0)
            else:
                curr_joint.set_damping(float(raw_dynamics.get("damping")))
            # store
            self.robot.add_joint(copy.deepcopy(curr_joint))
    ...
    def renumber_linksJoints(self, alpha_tie_breaker = False):
        ...
        root_link_name = list(link_names.difference(links_that_are_children))#[0]
        # start renumbering at -1 as the base link is fixed by default
        self.robot.get_links_ordered_by_id()[0].set_id(-1)
        # generate the standard dfs ordering of joints/links
        self.dfs_order_update(root_link_name, alpha_tie_breaker)
        # also save a bfs parse ordering and levels of joints/links
        self.bfs_order(root_link_name)
